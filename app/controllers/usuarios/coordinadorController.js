...
module.exports = {
  create: (req, res) => {
    let data = req.body;
    let coordinador = new Db({
      nombre: data.nombre,
      usuario: data.usuario
    });
    coordinador.save((error, resp) => {
      if (!error) {
  ...
  read: (req, res) => {
    ...
    Db.find().sort('nombre').paginate(page, item, (error, resp, total) => {
      if (!error) {
        Db.count((counterError, counter) => {
          if (counter > 0) {
            res.status(200).json({
  ...
  filter: (req, res) => {
    ...
    var params = req.body;
    Db.find(params).sort('nombre').paginate(page, item, (error, resp, total) => {
      if (!error) {
        Db.count((counterError, counter) => {
          if (counter > 0) {
            res.status(200).json({
