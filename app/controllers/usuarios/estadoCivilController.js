...
module.exports = {
  ...
  read: (req, res) => {
    ...
    Db.find().sort('estado-civil').paginate(page, item, (error, resp, total) => {
      if (!error) {
        Db.count((counterError, counter) => {
          if (counter > 0) {
            res.status(200).json({
  ...
  filter: (req, res) => {
    ...
    var params = req.body;
    Db.find(params).sort('estado-civil').paginate(page, item, (error, resp, total) => {
      if (!error) {
        Db.count((counterError, counter) => {
          if (counter > 0) {
            res.status(200).json({
